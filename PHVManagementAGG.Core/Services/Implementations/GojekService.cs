...
{
    public class GojekService : IGojekService
    {
        private readonly ISqlDataAccess db;
        private IConfiguration configuration;
        private readonly appSettings appSetting;        

        public GojekService(ISqlDataAccess db,
        ...
        public async Task<JObject> GetDataMenuGojekJsonStringAsync(string menuId)
        {                        
            var p = new DynamicParameters();
            p.Add("@menuVersionId", "1");
            p.Add("@menuId", "GJ_A");            
            p.Add("@platform", "GOJEK");
            p.Add("@enviroment", 0);
            p.Add("@rStatus", null, dbType: DbType.Boolean, direction: ParameterDirection.Output, 50);

            var result = (await db.QueryAsync<ShopeeMenuResponse, DynamicParameters>(command: "usp_MenuTool_BuilderByPlatformGojek_Jsonstring",
                                                     parameters: p,
                                                     connectionString: configuration.GetConnectionStringWithDecrypt("MenuToolPre"),
                                                     commandType: System.Data.CommandType.StoredProcedure)).ToList();
            if (!result.Any())
            {
                throw new BusinessException(ErrorCodes.DataNotFound, new object[] { "BuilderByPlatformGojek" });
            }
